...
type TabManager struct {
	...
	currentTab string // semantic ID of the most recently used tab
	mu         sync.RWMutex
}
...
func (tm *TabManager) TabContext(tabID string) (context.Context, string, error) {
	...
	// Return the semantic tab ID (not raw CDP ID) for external consistency
...
func (tm *TabManager) CreateTab(url string) (string, context.Context, context.CancelFunc, error) {
	...
	// Compute semantic tab ID and store only that entry.
	// Raw CDP ID is kept internal in the CDPID field.
	rawCDPID := string(targetID)
	semanticID := tm.idMgr.TabIDFromCDPTarget(rawCDPID)
	now := time.Now()

	tm.mu.Lock()
	tm.tabs[semanticID] = &TabEntry{Ctx: ctx, Cancel: cancel, CDPID: rawCDPID, CreatedAt: now, LastUsed: now}
	tm.accessed[semanticID] = true
	tm.currentTab = semanticID
	tm.mu.Unlock()

	return semanticID, ctx, cancel, nil
}
...
func (tm *TabManager) CloseTab(tabID string) error {
	...
	// Resolve semantic tab ID → raw CDP target ID for the actual CDP close call
...
// RegisterHashTab registers a semantic tab ID (e.g. "tab_<CDP_ID>") as an alias
// for the given raw CDP target ID. This allows callers to use the semantic ID for all
...
func (tm *TabManager) RegisterHashTab(hashID, rawCDPID string, ctx context.Context, cancel context.CancelFunc) {
	...
	tm.mu.Lock()
	defer tm.mu.Unlock()
	tm.tabs[hashID] = &TabEntry{Ctx: ctx, Cancel: cancel, CDPID: rawCDPID, CreatedAt: now, LastUsed: now}
